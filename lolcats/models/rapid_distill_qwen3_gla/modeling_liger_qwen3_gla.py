...
class LigerQwen3GatedLinearAttention(nn.Module):
    def __init__(
        ...
        # ----- output‑gate & normaliser-----
        self.use_output_gate = True                
        self.head_dim_out   = self.head_dim 

        # MLP that produces per‑token gating logits (same size as v)
        self.g_proj_s = nn.Linear(
            self.hidden_size,
    ...
    def destroy_teacher_weights(self):
        ...
        if self.layer_idx == 0:
            print("Destroying teacher weights from student model to prepare for Stage 2.")
        
        # Explicitly delete the nn.Parameter attributes to ensure memory is freed
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Check if we are in Stage 1 mode (teacher weights exist)

        if self.q_proj is not None:
            # Stage 1: Compute both teacher and student outputs for MSE loss
            with torch.no_grad():
            ...
            return o_teacher, special_attn_loss

        # Else, we are in Stage 2 mode (teacher weights have been destroyed)
        else:
            # Stage 2: Only compute student output. The output format must be standard.
            o_student, _, past_key_value = self.forward_student(hidden_states, attention_mask, position_ids, past_key_value,
                                                                 output_attentions, use_cache, cache_position, position_embeddings, **kwargs)
            
            # Return a standard attention output tuple. `attention_output` is None because
            # we don't need to return special values for the KDTrainer.
            return o_student, None, past_key_value
    ...
    def forward_teacher(
        ...
        **kwargs,
        ...
        o = _flash_attention_forward(  # Reashape to the expected shape for Flash Attention
            q.transpose(1, 2),
            k.transpose(1, 2),
            v.transpose(1, 2),
            attention_mask,  # should be None
            q_len,
...
class LigerQwen3GLAModel(Qwen3Model, LigerQwen3PreTrainedModel):
    ...
    def __init__(self, config: LigerQwen3GLAConfig):
        ...
        self.layers = nn.ModuleList(
            [LigerQwen3DecoderLayer(config, layer_idx)
...
class LigerQwen3GLAForCausalLM(LigerQwen3PreTrainedModel, Qwen3ForCausalLM, GenerationMixin):
    ...
    def destroy_teacher_weights(self):
        for layer in self.model.layers:
            layer.self_attn.destroy_teacher_weights()